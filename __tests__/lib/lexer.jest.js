...
const { lex, TOKEN_TYPE, UNRECOGNIZABLE_TOKEN_MESSAGE } = lexer
...
describe('lex', () => {
  ...
  describe('basic identifiers', () => {
    ...
    test('allows names starting with underscores', () => {
      ...
      expect(result).toEqual([{ type: TOKEN_TYPE.IDENTIFIER, name: '___beau' }])
    })
    ...
    test('takes the longest munch of identifier names ignoring int keyword prefixes', () => {
      ...
      expect(result).toEqual([{ type: TOKEN_TYPE.IDENTIFIER, name: 'integer' }])
    })
  ...
  describe('constant numbers', () => {
    test('identifies simple integer constants', () => {
      ...
      expect(result).toEqual([{ type: TOKEN_TYPE.CONSTANT, constant: '9999' }])
    })
  })
